...
namespace MovementTemplates.Scripts
{
    ...
    public class Movement_Audio : MonoBehaviour
    {
        ...
        [SerializeField] GameObject goAudioManager; //audio
        ...
        void Start()
        {
            ...
            AudioManager = goAudioManager.GetComponent<AudioManager>(); //audio
        }

        //audio
        private void Update()
        {            
            if (Input.GetButtonDown("Jump") && this.IsColliding(Vector2.down))
            {
                int i = Random.Range(1, 4);
                string soundToPlay = "Jump" + i; 
